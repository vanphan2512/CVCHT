...
export class UserLogin extends BasePage {
    ...
    private login(user: any, password: any) {
        ...
        userService.signin(userModel).then(function (token: any) {
            ...
            self.router.navigate([configHelper.getAppConfig().businessUrl]);
            localStorage.setItem(configHelper.getAppConfig().businessUrl + "_notify", "false");
            session.set(CACHE_CONSTANT.CURRENT_USER, user);
            session.set(CACHE_CONSTANT.CURRENT_PASSWORD, password);
