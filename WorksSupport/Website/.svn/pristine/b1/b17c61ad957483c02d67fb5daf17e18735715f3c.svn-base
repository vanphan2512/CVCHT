...
export class GridModel {
    ...
    public getColumns(): Array<any> {
        ...
        let row: any;
        let meta: any;
        let isCanEdit: boolean = session.get(CACHE_CONSTANT.EO_WORKSSUPORTGROUP_EDIT) === "true" ? true : false;
        let isCanDelete: boolean = session.get(CACHE_CONSTANT.EO_WORKSSUPORTGROUP_DELETE) === "true" ? true : false;
        let isCanAdd: boolean = session.get(CACHE_CONSTANT.EO_WORKSSUPORTGROUP_ADD) === "true" ? true : false;
        ...
        this.options.columns.forEach(function (column: any, index: number) {
            switch (column.type) {
                ...
                case "color": {
                    columns.push({ "data": column.field, title: column.title, "targets": index, "defaultContent": !!column.content ? column.content : String.empty, "render": function (data: any, type: any, full: any, meta: any) { return "<i class=' fa fa-square' aria-hidden='true' style='font-size: 22px; color:" + data + "'></i>"; }, "className": !!column.className ? column.className : null, "width": !!column.width ? column.width : null, "visible": column.visible === "false" ? false : true, "orderable": column.sort === "false" ? false : true });
                    break;
                }
                case "toggle": {
