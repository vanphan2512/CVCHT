...
import { ValidationDirective, SelectStatus } from "../../../common/directive";
...
@Component({
    ...
    directives: [ErrorMessage, ValidationDirective, SelectStatus]
})
export class Permission extends BasePage {
    ...
    public isCanView: boolean = sessionStorage.get(CACHE_CONSTANT.EO_WORKSSUPPORT_AUTHORIZE_VIEW) === "true" ? true : false;
    public isCanEdit: boolean = sessionStorage.get(CACHE_CONSTANT.EO_WORKSSUPPORT_AUTHORIZE_EDIT) === "true" ? true : false;
    ...
    // List WorksType
    public listWorkTypeForCopy: Array<any> = [];
    // List MemberRole
    ...
    public memberRoleModel: MemberRoleModel;
    public wfPermissModel: WfPermissModel;
    ...
    public defaultRoleSelected: number = 0;
    ...
    constructor(router: Router) {
        ...
        self.getAllMembersByDepart();
    }

    ngOnInit() {
        this.memberRoleModel = new MemberRoleModel(0, 0, false, false, false, false, false, false, false, false);
        this.wfPermissModel = new WfPermissModel(0, 0, false, false, false, false, false, false, false);
        // this.defaultRoleSelected = new MemberRoleModel(0, 0, false, false, false, false, false, false, false, false);
        ...
        this.isCanView = sessionStorage.get(CACHE_CONSTANT.EO_WORKSSUPPORT_AUTHORIZE_VIEW) === this.isCanView ? true : false;
        this.isCanEdit = sessionStorage.get(CACHE_CONSTANT.EO_WORKSSUPPORT_AUTHORIZE_EDIT) === this.isCanEdit ? true : false;
    ...
    public refreshPage(worksTypeId: any) {
        ...
        worksTypeService.get(worksTypeId).then(function (item: any) {
            ...
            memberService.getMembersActived().then(function (items: Array<any>) {
                ...
                self.currentMemberRolePermisId = items[0].WorksSupportMemberRoleId;
                self.currentMemberRoleId = items[0].WorksSupportMemberRoleId;
            });

            self.permissionDetail = item;
            ...
            if (permissModel != null) {
                ...
                self.wfPermissModel = new WfPermissModel(
                    self.ListWorksSupportTypeWorkFlow[0].WorksSupportStepId, 0, false, false, false, false, false, false, false,
                );
            }

            let currentMemberRoleId = self.currentMemberRolePermisId;
            let memberRoleModel: MemberRoleModel = null;
            if (self.permissionDetail.ListWorksSupportTypeMemberRole.length > 0) {
                let listMemberRole: Array<any> = self.permissionDetail.ListWorksSupportTypeMemberRole;
                let index = listMemberRole.findIndex(obj => obj.WorksSupportMemberRoleId.toString() === currentMemberRoleId.toString());
                if (index > -1) {
                    let memberRole = self.permissionDetail.ListWorksSupportTypeMemberRole[index];
    ...
    private loadWorkType() {
        ...
        worksTypeService.getWorksTypesActived().then(function (items: Array<any>) {
            if (items.length > 0) {
                ...
                self.WorksTypeId = worksTypeId;
                self.getWorksTypeDetail(worksTypeId);
            }
        });
    }
    ...
    private getWorksTypeDetail(worksTypeId: any) {
        let self: Permission = this;
        let $ = window.jQuery;
        ...
        worksTypeService.get(worksTypeId).then(function (item: any) {
            ...
            memberService.getMembersActived().then(function (items: Array<any>) {
                ...
                self.permissionDetail = item;
                self.currentTypeID = self.WorksTypeId;
                self.ListWorksSupportTypeMemberRole = item.ListWorksSupportTypeMemberRole == null ? [] : item.ListWorksSupportTypeMemberRole;
                self.ListWorksSupportTypeWorkFlow = item.ListWorksSupportTypeWorkFlow == null ? [] : item.ListWorksSupportTypeWorkFlow;
                self.currentStepID = self.ListWorksSupportTypeWorkFlow.length == 0 ? 0 : self.ListWorksSupportTypeWorkFlow[0].WorksSupportStepId;
                self.ListWorksSupportProjectPermis = item.ListWorksSupportProjectPermis == null ? [] : item.ListWorksSupportProjectPermis;
                if (self.ListWorksSupportTypeMemberRole !== null && self.ListWorksSupportTypeMemberRole.length > 0) self.currentMemberRolePermisId = self.ListWorksSupportTypeMemberRole[0].WorksSupportMemberRoleId;
                else self.currentMemberRolePermisId = "";
                self.ListWorksSupportTypeWfNx = self.ListWorksSupportTypeWorkFlow == null ? [] : (self.ListWorksSupportTypeWorkFlow[0].ListWorksSupportTypeWfNx == null ? [] : self.ListWorksSupportTypeWorkFlow[0].ListWorksSupportTypeWfNx);
                self.ListWorksSupportTypeWfNxPermis = self.ListWorksSupportTypeWorkFlow == null ? [] : (self.ListWorksSupportTypeWorkFlow[0].ListWorksSupportTypeWfNxPermis == null ? [] : self.ListWorksSupportTypeWorkFlow[0].ListWorksSupportTypeWfNxPermis);
                self.ListWorksSupportTypeWfPermis = self.ListWorksSupportTypeWorkFlow == null ? [] : (self.ListWorksSupportTypeWorkFlow[0].ListWorksSupportTypeWfPermis == null ? [] : self.ListWorksSupportTypeWorkFlow[0].ListWorksSupportTypeWfPermis);
                self.test = self.ListWorksSupportTypeWorkFlow == null ? [] : (self.ListWorksSupportTypeWorkFlow[0].ListWorksSupportTypeWfPermis == null ? [] : self.ListWorksSupportTypeWorkFlow[0].ListWorksSupportTypeWfPermis);
                let indexPermis = self.ListWorksSupportTypeWfPermis.findIndex(_ => _.WorksSupportMemberRoleId.toString() === self.currentMemberRolePermisId.toString());
                let permissModel = self.ListWorksSupportTypeWfPermis.length == 0 ? null : (indexPermis < 0 ? null : self.ListWorksSupportTypeWfPermis[indexPermis]);
                if (permissModel != null) {
                    self.wfPermissModel = new WfPermissModel(
                        permissModel.WorksSupportStepId,
                        permissModel.WorksSupportMemberRoleId,
                    ...
                    self.wfPermissModel = new WfPermissModel(
                        self.ListWorksSupportTypeWorkFlow[0].WorksSupportStepId, self.currentMemberRolePermisId, false, false, false, false, false, false, false
                    );
                }
                if (self.ListWorksSupportTypeMemberRole.length > 0) {
                    let listMemberRole: Array<any> = self.ListWorksSupportTypeMemberRole;
                    let index = listMemberRole.findIndex(obj => obj.WorksSupportMemberRoleId.toString() === self.currentMemberRoleId.toString());
                    if (index > -1) {
                        let memberRole = self.ListWorksSupportTypeMemberRole[index];
                        ...
                        self.memberRoleModel = new MemberRoleModel(
                            self.permissionDetail.WorksSupportTypeId, self.currentMemberRoleId, false, false, false, false, false, false, null, false
                        );
                    }
                }
    ...
    public getRoleDetail(event: any) {
        ...
        if (event.target.localName == "a") {
            ...
            if (checkboxid != "" && checkboxid === self.defaultRoleSelected.toString()) {
                self.defaultRoleSelected = 0;
            }
            let index = self.ListWorksSupportTypeMemberRole.findIndex(obj => obj.WorksSupportMemberRoleId.toString() === checkboxid.toString());
            let idx = self.ListWorksSupportTypeWfPermis.findIndex(obj => obj.WorksSupportMemberRoleId.toString() === checkboxid.toString())
            if (event.target.checked == false) {
                if (index > -1) {
                    self.ListWorksSupportTypeMemberRole.removeItem(self.ListWorksSupportTypeMemberRole[index]);
                }
                if (idx > -1) {
                    self.ListWorksSupportTypeWfPermis.removeItem(self.ListWorksSupportTypeWfPermis[idx]);
                }
                if (self.currentMemberRolePermisId.toString() === checkboxid.toString()) self.ListWorksSupportTypeWfNxPermis = [];
                let indexWF: any = self.ListWorksSupportTypeWorkFlow.findIndex(_ => _.WorksSupportStepId.toString() === self.currentStepID.toString());
                self.ListWorksSupportTypeWorkFlow[indexWF].ListWorksSupportTypeWfPermis = self.ListWorksSupportTypeWfPermis;
                self.ListWorksSupportTypeWorkFlow[indexWF].ListWorksSupportTypeWfNxPermis = self.ListWorksSupportTypeWfNxPermis;
                if (self.currentMemberRoleId.toString() === checkboxid.toString()) {
                    self.memberRoleModel = new MemberRoleModel(
                        self.permissionDetail.WorksSupportTypeId, parseInt(RoleID), false, false, false, false, false, false, RoleName, false
                    );
                }
            }
            else {
        ...
        self.ListWorksSupportTypeMemberRole.sort(function(a,b){
            return a.WorksSupportMemberRoleId-b.WorksSupportMemberRoleId
        });
    }
    ...
    public getMemeberRoleOfProcess(event: any) {
        ...
        if (event.target.localName == "a") {
            ...
            this.SavePerMission();
            self.ListWorksSupportTypeWfPermis = self.ListWorksSupportTypeWorkFlow[indexWorksFlow].ListWorksSupportTypeWfPermis;
            self.ListWorksSupportTypeWfNxPermis = self.ListWorksSupportTypeWorkFlow[indexWorksFlow].ListWorksSupportTypeWfNxPermis;
            ...
            if (index > -1) {
                ...
                self.wfPermissModel = new WfPermissModel(
                    self.currentStepID, checkId, false, false, false, false, false, false, false
                );
            }
            ...
            if (self.currentMemberRolePermisId.toString() === checkboxid.toString()) {
                if (event.target.checked == false) {
                    ...
                    self.ListWorksSupportTypeWfNxPermis.filter(_ => _.WorksSupportMemberRoleId.toString() === checkboxid.toString()).forEach(function (element: any) {
                        self.ListWorksSupportTypeWfNxPermis.removeItem(element);
                    });
                    let indexWF: any = self.ListWorksSupportTypeWorkFlow.findIndex(_ => _.WorksSupportStepId.toString() === self.currentStepID.toString());
                    self.ListWorksSupportTypeWorkFlow[indexWF].ListWorksSupportTypeWfPermis = self.ListWorksSupportTypeWfPermis;
                    self.ListWorksSupportTypeWorkFlow[indexWF].ListWorksSupportTypeWfNxPermis = self.ListWorksSupportTypeWfNxPermis;
                    self.wfPermissModel = new WfPermissModel(
                        self.currentStepID, checkboxid, false, false, false, false, false, false, false
                    );
                }
                else {
                    if (self.wfPermissModel !== null && self.wfPermissModel.WorksSupportMemberRoleId.toString() !== checkboxid.toString()) self.wfPermissModel = new WfPermissModel(
                        self.currentStepID, checkboxid, false, false, false, false, false, false, false
                    );
                    let index = self.ListWorksSupportTypeWfPermis.findIndex(obj => obj.WorksSupportStepId.toString() === self.currentStepID.toString() && obj.WorksSupportMemberRoleId.toString() === checkboxid.toString());
                    if (index === -1) {
                        self.ListWorksSupportTypeWfPermis.push(self.wfPermissModel);
                    }
                    let indexWF: any = self.ListWorksSupportTypeWorkFlow.findIndex(_ => _.WorksSupportStepId.toString() === self.currentStepID.toString());
                    self.ListWorksSupportTypeWorkFlow[indexWF].ListWorksSupportTypeWfPermis = self.ListWorksSupportTypeWfPermis;
                }
                ...
                if (event.target.checked == false) {
                    ...
                    let permis: WfPermissModel = new WfPermissModel(
                        self.currentStepID, checkboxid, false, false, false, false, false, false, false
                    );
                    let index = self.ListWorksSupportTypeWfPermis.findIndex(obj => obj.WorksSupportStepId.toString() === self.currentStepID.toString() && obj.WorksSupportMemberRoleId.toString() === checkboxid.toString());
                    if (index === -1) {
                        self.ListWorksSupportTypeWfPermis.push(permis);
                    }
                    let indexWF: any = self.ListWorksSupportTypeWorkFlow.findIndex(_ => _.WorksSupportStepId.toString() === self.currentStepID.toString());
                    self.ListWorksSupportTypeWorkFlow[indexWF].ListWorksSupportTypeWfPermis = self.ListWorksSupportTypeWfPermis;
                }
    ...
    public SavePerMission() {
        let $ = window.jQuery;
        let self: Permission = this;
        let indexWfPermis: any = self.ListWorksSupportTypeWfPermis.findIndex(obj => obj.WorksSupportStepId.toString() === self.currentStepID.toString() && obj.WorksSupportMemberRoleId.toString() === self.currentMemberRolePermisId.toString())
        let indexWfNxPermis: any = self.ListWorksSupportTypeWfNxPermis.findIndex(_ => _.WorksSupportStepId.toString() === self.currentStepID.toString() && _.WorksSupportMemberRoleId.toString() === self.currentMemberRolePermisId.toString())
        if ($('#roleOfPermissionProcess #MemberRoleProcess-' + self.currentMemberRolePermisId)[0] !== undefined) {
            if ($('#roleOfPermissionProcess #MemberRoleProcess-' + self.currentMemberRolePermisId)[0].children[0].children[0].checked == false) {
                if (indexWfPermis > -1) {
                    let objPermission = self.ListWorksSupportTypeWfPermis[indexWfPermis];
                    self.ListWorksSupportTypeWfPermis.removeItem(objPermission);
                }
                if (indexWfNxPermis > -1) {
                    let objPermissionNx = self.ListWorksSupportTypeWfNxPermis[indexWfNxPermis];
                    self.ListWorksSupportTypeWfNxPermis.removeItem(objPermissionNx);
                }
            }
            else {
                if (indexWfPermis > -1) {
                    if (self.wfPermissModel.WorksSupportMemberRoleId !== -1)
                        self.ListWorksSupportTypeWfPermis[indexWfPermis] = self.wfPermissModel;
                }
                else { if (self.wfPermissModel.WorksSupportMemberRoleId !== -1) self.ListWorksSupportTypeWfPermis.push(self.wfPermissModel); }
            }
            let indexWF: any = self.ListWorksSupportTypeWorkFlow.findIndex(_ => _.WorksSupportStepId.toString() === self.currentStepID.toString());
            self.ListWorksSupportTypeWorkFlow[indexWF].ListWorksSupportTypeWfPermis = self.ListWorksSupportTypeWfPermis;
            self.ListWorksSupportTypeWorkFlow[indexWF].ListWorksSupportTypeWfNxPermis = self.ListWorksSupportTypeWfNxPermis;
    ...
    public UpdateMemberRole() {
        ...
        if ($('#ulListMemberRole #MemberRole-' + self.currentMemberRoleId)[0].children[0].children[0].checked == false) {
            ...
            {   
                let name = $('#ulListMemberRole #MemberRole-' + self.currentMemberRoleId)[0].innerText.trim();
                self.memberRoleModel.WorksSupportMemberRoleName = name;
                self.ListWorksSupportTypeMemberRole.push(self.memberRoleModel);
            }
        }
        self.isSaved = false;
    }
    ...
    CheckSelectedMemberRole(item: any): boolean {
        let self: Permission = this;
        let $ = window.jQuery;
        let wfPermis = self.ListWorksSupportTypeWfPermis.length === 0 ? false : (self.ListWorksSupportTypeWfPermis.findIndex(obj => obj.WorksSupportMemberRoleId.toString() === item.toString() && obj.WorksSupportStepId.toString() === self.currentStepID.toString()) > -1 ? true : false);
        let wfNxPermis = self.ListWorksSupportTypeWfNxPermis.length === 0 ? false : (self.ListWorksSupportTypeWfNxPermis.findIndex(obj => obj.WorksSupportMemberRoleId.toString() === item.toString() && obj.WorksSupportStepId.toString() === self.currentStepID.toString()) > -1 ? true : false);
        if (wfPermis === false && wfNxPermis === false)
            return false;
        else return true;
    }
    ...
    public getNextStep(event: any) {
        ...
        self.ListWorksSupportTypeWfNx = self.ListWorksSupportTypeWorkFlow == null ? [] : (self.ListWorksSupportTypeWorkFlow[index].ListWorksSupportTypeWfNx == null ? [] : self.ListWorksSupportTypeWorkFlow[index].ListWorksSupportTypeWfNx);
        self.ListWorksSupportTypeWfNxPermis = self.ListWorksSupportTypeWorkFlow == null ? [] : (self.ListWorksSupportTypeWorkFlow[index].ListWorksSupportTypeWfNxPermis == null ? [] : self.ListWorksSupportTypeWorkFlow[index].ListWorksSupportTypeWfNxPermis);
        self.ListWorksSupportTypeWfPermis = self.ListWorksSupportTypeWorkFlow == null ? [] : (self.ListWorksSupportTypeWorkFlow[index].ListWorksSupportTypeWfPermis == null ? [] : self.ListWorksSupportTypeWorkFlow[index].ListWorksSupportTypeWfPermis);
        let indexPermis = self.ListWorksSupportTypeWfPermis.findIndex(_ => _.WorksSupportMemberRoleId.toString() === self.currentMemberRolePermisId.toString());
        let permissModel = self.ListWorksSupportTypeWfPermis.length == 0 ? null : (indexPermis < 0 ? null : self.ListWorksSupportTypeWfPermis[indexPermis]);
        if (permissModel != null) {
            self.wfPermissModel = new WfPermissModel(
            ...
            self.wfPermissModel = new WfPermissModel(
                self.ListWorksSupportTypeWorkFlow[index].WorksSupportStepId, self.currentMemberRolePermisId, false, false, false, false, false, false, false
            );
        }
    }
    ...
    public resetForm() {
        ...
        let self: Permission = this;
        self.wfPermissModel = new WfPermissModel(
            self.currentStepID, 0, false, false, false, false, false, false, false
        );
        self.memberRoleModel = new MemberRoleModel(
            self.permissionDetail.WorksSupportTypeId, 0, false, false, false, false, false, false, null, false
        );
        ...
        $("#txtWorksFlow").val('');
        $("#textRole").val('');
        $("#selectDefaultRole").removeClass("has-error");
    }
     ...
     * @memberof Permission
     */
    public onSaveConfirmClick() {
        let $ = window.jQuery;
        let self: Permission = this;
        if (self.ListWorksSupportTypeMemberRole.length === 0) {
            $('#errorNullMemberRole').modal('show');
            $("#selectDefaultRole").removeClass("has-error");
        }
        else if (self.defaultRoleSelected == 0) {
            $("#selectDefaultRole").addClass("has-error");
            $('#errorNullDefaultRole').modal('show');
        }
        else {
            worksTypeService.CheckWorkTypeIsUsed(self.WorksTypeId).then(function (items: any) {
                if (items.IsUsed === 1) {
                    $('#confirmationSaveModal').modal('show');
                }
                else self.onSaveClick();
            });
            $("#selectDefaultRole").removeClass("has-error");
        }
    }
    // Remove class has-error for select Default Role
    ...
    public onSaveClick() {
        ...
        worksTypeService.createPermission(self.permissionDetail).then(function (items: Array<any>) {
            ...
            configHelper.showNotify("edit", false);
        });
    }
    ...
    public resetData(event: any) {
        let self: Permission = this;
        let $ = window.jQuery;
        self.getWorksTypeDetail(self.WorksTypeId);
        this.memberRoleModel = new MemberRoleModel(0, 0, false, false, false, false, false, false, false, false);
        this.lstAddMemberOfDepart = [];
        $("#txtSearchMember").val('');
        $("#keywordsearchbox").val('');
    ...
    public popupCopyPermission(event: any) {
        ...
        worksTypeService.getWorksTypesActived().then(function (items: Array<any>) {
            if (items.length > 0) {
                self.listWorkTypeForCopy = items;
            }
        });
    ...
    public copyPermission(event: any) {
        ...
        $('#popCopyPermission').modal('hide');
        worksTypeService.copyPermission(model).then(function (items: Array<any>) {
            configHelper.showNotify("copy", true);
        }).error(function () {
            configHelper.showNotify("copy", false);
        });
    }
    public checkboxRoleID(event: any) {
        let self: Permission = this;
        let roleID = event.target.id.replace('CB-', '');
        if (event.target.checked === false) {
            let index = self.ListWorksSupportTypeWfPermis.findIndex(obj => obj.WorksSupportStepId.toString() === self.currentStepID.toString() && obj.WorksSupportMemberRoleId.toString() === roleID.toString())
            if (index > -1) {
                self.ListWorksSupportTypeWfPermis.removeItem(self.ListWorksSupportTypeWfPermis[index]);
                let indexWF: any = self.ListWorksSupportTypeWorkFlow.findIndex(_ => _.WorksSupportStepId.toString() === self.currentStepID.toString());
                self.ListWorksSupportTypeWorkFlow[indexWF].ListWorksSupportTypeWfPermis = self.ListWorksSupportTypeWfPermis;
            }
            if (self.currentMemberRolePermisId.toString() === roleID.toString()) {
                self.wfPermissModel = new WfPermissModel(
                    self.currentStepID, -1, false, false, false, false, false, false, false
                );
            }
        }
    }
