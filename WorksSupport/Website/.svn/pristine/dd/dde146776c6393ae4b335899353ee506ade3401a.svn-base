...
export class Members extends BasePage {
    ...
    public onDeleteMembers(event: any) {
        let $ = window.jQuery;
        let self: Members = this;
        memberService.checkRoleIsUsed(this.objDelete.Id).then(function (item: any) {
            // check number of role >0
            if (item.NumberOfRole === 0) {
                memberService.delete(self.objDelete).then(function (event: any) {
                    $("#modal").modal("hide");
                ...
                configHelper.showNotify("using", false);
            }
        });
    }
    ...
    public isValid(isAdd: boolean): boolean {
        ...
        if (!isAdd) {
            if (orderIndex < 1 || orderIndex % 1 != 0 || orderIndex > 999) {
                validationErrors.add("OrderIndex", [], "Thứ tự vai trò không hợp lệ!");
                $("#OrderIndex").addClass("has-error");
            }
        }
        if (!this.WorksSupportMemberRoleName) {
            validationErrors.add("WorksSupportMemberRoleName", [], "Tên vai trò là bắt buộc!");
            $("#WorksSupportMemberRoleName").addClass("has-error");
        }
        if (lenName > 200) {
            validationErrors.add("WorksSupportMemberRoleName", [], "Tên vai trò không được vượt quá 200 kí tự!");
            $("#WorksSupportMemberRoleName").addClass("has-error");
        }
        if (lenDesc > 2000) {
    ...
    public onEditMember(event: any) { 
        ...
        if (this.isValid(false)) {
            // nếu tắt kích hoạt thì kiểm tra vai trò đã được dùng chưa
            if (isActived === false) {
                memberService.checkRoleIsUsed(memberId).then(function (item: any) { 
                    if (item.NumberOfRole === 0) {
    ...
    public validateOnServer(error: any) {
        let $ = window.jQuery;
