...
namespace Nc.Erp.WorksSupport.Da.Configuration.Project
{
    ...
    public class DaWorksSupportProject
    {
        ...
        public ResultMessage GetWorksSupportType(ref List<WorksSupportType> list, string userName)
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    if (!Convert.IsDBNull(reader["ISDEFAULTROLE"])) objWorksSupportType.IsDefaultRole = Convert.ToInt32(reader["ISDEFAULTROLE"]);
        ...
        /// <summary>
        /// lấy tất cả dự án theo username
        /// </summary>
        /// <param name="list"></param>
        /// <param name="userName"></param>
        /// <returns></returns>
        public ResultMessage GetProjectByMemberId(ref List<WorksSupportProject> list, string userName)
        {
            var objResultMessage = new ResultMessage();
            var objIData = Data.CreateData();
            try
            {
                objIData.Connect();
                objIData.CreateNewStoredProcedure(SpSelectProjectByUserName);
            ...
            {
                objResultMessage = new ResultMessage(true, SystemError.ErrorTypes.LoadInfo,
                    "Lỗi nạp thông tin danh sách WorksSupportProject theo userName", objEx.ToString());
                ErrorLog.Add(objIData, objResultMessage.Message, objResultMessage.MessageDetail,
                    "DaWorksSupportProject -> GetProjectByMemberId", "");
                return objResultMessage;
            }
            finally
            {
        ...
        /// <param name="projectId"></param>
        /// <returns></returns>
        public ResultMessage CheckUserIsProccess(ref WorksSupportProject objProject , string userName, int projectId)
        {
            var objResultMessage = new ResultMessage();
            var objIData = Data.CreateData();
            try
            {
                ...
                if (reader.Read())
                {
                    if (!Convert.IsDBNull(reader["CHECKPROCESSUSER"])) objProject.NumberOfMember = Convert.ToInt32(reader["CHECKPROCESSUSER"]);
                }
            }
            catch (Exception objEx)
            {
                ...
                ErrorLog.Add(objIData, objResultMessage.Message, objResultMessage.MessageDetail,
                    "DaWorksSupportProject -> CheckUserIsProccess", "");
                return objResultMessage;
            }
            finally
            {
        ...
        public const string SpSelectProjectByUserName = "WORKSSUPORTPROJECT_V2_ISMEMBER";
        public const string SpSelectCheckUserWorkflow = "WORKSSUPORTPROJECT_V2_CHK_USER";
