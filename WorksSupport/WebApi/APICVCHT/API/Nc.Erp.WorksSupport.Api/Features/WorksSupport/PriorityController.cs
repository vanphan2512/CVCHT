    ...
    public class PriorityController : BaseController
    {
        ...
        public Response<WorksSupportPriority> Delete([FromBody] DeleteParam objparam)
        {
            ...
            try
            {
                ...
                new DaWorksSupportPriority().CheckPriorityIsUsed(id, ref isUsed);
                if (isUsed)
                {
                    var error = new ValidationError
                    {
                        Key = "PriorityIsUsed",
                        Message = "Tên độ ưu tiên đã được sử dụng không thể xóa!"
                    };
                    listError.Add(error);
                }
        ...
        public Response<WorksSupportPriority> SaveDataBy([FromBody] SaveParamBy objparam)
        {
            ...
            try
            {
                ...
                {
                    new DaWorksSupportPriority().CheckPriorityIsUsed(objparam.Id, ref isUsed);
                }
                ...
                if (isUsed)
                {
                    ...
                    {
                        Key = "PriorityIsUsed",
                        Message = "Tên độ ưu tiên đã được sử dụng không thể bỏ kích hoạt!"
                    };
                    listError.Add(error);
                }
                if (listError.Count > 0)
