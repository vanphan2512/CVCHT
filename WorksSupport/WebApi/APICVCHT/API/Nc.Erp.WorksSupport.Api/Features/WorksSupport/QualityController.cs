    ...
    public class QualityController : BaseController
    {
        ...
        public Response<WorksSupportQuality> Delete([FromBody] DeleteParam objparam)
        {
            ...
            try
            {
                ...
                new DaWorksSupportQuality().CheckQualityIsUsed(id, ref isUsed);
                if (isUsed)
                {
                    var error = new ValidationError
                    {
                        Key = "QuanlityIsUsed",
                        Message = "Chất lượng đã được sử dụng không thể xóa!"
                    };
                    listError.Add(error);
                }
        ...
        public Response<WorksSupportQuality> SaveDataBy([FromBody] SaveParamBy objparam)
        {
            ...
            try
            {
                ...
                var listError = new List<ValidationError>();
                bool isUsed = false;
                if (objparam.Id > 0 && objparam.IsActived == 0)
                {
                    new DaWorksSupportQuality().CheckQualityIsUsed(objparam.Id, ref isUsed);
                }
                ...
                if (objCheck.CountName >= 1)
                {
                    ...
                    error.Message = "Tên chất lượng đã tồn tại!";
                    listError.Add(error);
                }
                ...
                if (isUsed)
                {
                    ...
                    {
                        Key = "QuanlityIsUsed",
                        Message = "Chất lượng đã được sử dụng không thể bỏ kích hoạt!"
                    };
                    listError.Add(error);
                }
                if (listError.Count > 0)
        ...
        public Response<List<WorksSupportQuality>> QualitiesActived([FromUri] WorksParam param)
        {
            // Definition object to return.
            var ret = new Response<List<WorksSupportQuality>>();
            try
            {
                ...
                // Call DaSystemManagement to search data.
                var objResultMessage = new DaWorksSupportQuality().GetQualityByWorksId(ref listWorksSupportQuality, worksSupportId );
