    ...
    public class WorkController : BaseController
    {
        ...
        public Response<WorkSupport> SaveDataBy([FromBody] SaveWorksParam objparam)
        {
            ...
            try
            {
                ...
                ValidateParams(objparam);
                // Definition object return
                var numeberOfWorkName = 0;
                var listError = new List<ValidationError>();
                new DaWorks().CheckWorkName(ref numeberOfWorkName, objparam.Id, objparam.WorksSupportGroupId, objparam.WorksSupportName);
                if (numeberOfWorkName >= 1)
                {
                    var error = new ValidationError { Key = "WorksSupportName", Message = "Tên công việc đã tồn tại!" };
                    listError.Add(error);
                    ret.Success = false;
                ...
                // Call DaWorksSupportStatus to search data.
                var objResultMessage = new DaWorksSupport().InsertWorksSupport(obj, objparam.ListInvoleId, ref obj, objparam.User, objparam.ListAttachment, objparam.ListDeletedAttachment);
        ...
        public Response<WorkSupport> AddSolutionWorksSupport([FromBody] SaveSolutionParam objparam)
        {
            ...
            try
            {
                ...
                // Cập nhật đánh giá chất lượng công việc
                new DaWorks().AddSolution(objparam.WorksSupportId, objparam.SolutionContent, objparam.User, objparam.ListSolutionAttachment, objparam.IsAddNew);
                ret.Success = true;
                return ret;
            }
            catch (Exception e)
        ...
        [HttpPost]
        [Route("deleteSolutionAttachment")]
