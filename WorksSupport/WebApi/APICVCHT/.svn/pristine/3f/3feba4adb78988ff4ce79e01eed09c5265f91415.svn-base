...
namespace Nc.Erp.WorksSupport.Da.Configuration.Status
{
    ...
    public class DaWorksSupportStatus
    {
        ...
        public ResultMessage CheckAllStatus(ref StatusCheck objWorksSupportStatus)
        {
            var objResultMessage = new ResultMessage();
            var objIData = Data.CreateData();
            try
            {
                objIData.Connect();
                objIData.CreateNewStoredProcedure(SpCheckAll);
                var reader = objIData.ExecStoreToDataReader();
                if (reader.Read())
                {
                    if (!Convert.IsDBNull(reader["ISCLOSESTATUS"])) objWorksSupportStatus.IsCloseStatus = Convert.ToBoolean(reader["ISCLOSESTATUS"]);
                    if (!Convert.IsDBNull(reader["ISCOMPLETESTATUS"])) objWorksSupportStatus.IsCompleteStatus = Convert.ToBoolean(reader["ISCOMPLETESTATUS"]);
                    if (!Convert.IsDBNull(reader["ISINITSTATUS"])) objWorksSupportStatus.IsInitStatus = Convert.ToBoolean(reader["ISINITSTATUS"]);
                    if (!Convert.IsDBNull(reader["TOTAL"])) objWorksSupportStatus.Total = Convert.ToInt32(reader["TOTAL"]);
                }
        ...
        /// Description     : checking WORKSSUPPORTPRIORITYID is used.
        /// </summary>
        /// <param name="intWorksSupportStatusId"></param>
        /// <param name="isUsed"></param>
        /// <returns></returns>
        public ResultMessage CheckStatusIsUsed(int intWorksSupportStatusId, ref bool isUsed)
        {
            var objResultMessage = new ResultMessage();
            var objIData = Data.CreateData();
            try
            {
                objIData.Connect();
                objIData.CreateNewStoredProcedure(SpCheckStatusIsUsed);
            ...
            {
                objResultMessage = new ResultMessage(true, SystemError.ErrorTypes.LoadInfo,
                    "Lỗi kiểm tra trạng thái đã sử dụng", objEx.ToString());
                ErrorLog.Add(objIData, objResultMessage.Message, objResultMessage.MessageDetail,
                    "DaWorksSupportStatus -> CheckStatusIsUsed", "");
                return objResultMessage;
            }
            finally
            {
        ...
        public const string SpCheckAll = "WORKSSUPPORTSTATUS_V2_CHECKALL";
        public const string SpCheckStatusIsUsed = "EO_WS_STATUS_CHK_USED";
        #endregion
    }
}
